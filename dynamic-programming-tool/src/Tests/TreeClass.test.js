import { TreeNode, Tree } from '../Classes/TreeClass';
...
test('TreeNode contains works for true', () => {
    const randVal = Math.floor(Math.random() * 10) + 1
    const node = new TreeNode(randVal)
    const nodeID = node.ID
...
test('TreeNode contains works for false', () => {
    const randVal = Math.floor(Math.random() * 10) + 1
    const node = new TreeNode(randVal)
    const nodeID = node.ID
    ...
    expect(node.containsNodeVal(100)).toBe(false)
})

test('TreeNode serialize tree works for ', () => {
...
test('TreeNode decompose tree works', () => {
    ...
    expect(tree.root.decomposeTree().toString).toEqual(expected.toString)
})